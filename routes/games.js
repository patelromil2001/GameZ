...
router.get('/', async (req, res) => {
    try {
        ...
        if (req.query.sort) {
            ...
            const allowedSortFields = ['name', 'price', 'release_date', 'positive', 'negative'];
        ...
        if (req.query.genres) {
            const genresList = req.query.genres.split(',').map(genre => genre.trim());

            // Create an array of regex patterns for each genre
            const genrePatterns = genresList.map(genre =>
                new RegExp(`'${genre}'`, 'i')  // Match the genre with single quotes around it
            );
        ...
        if (req.query.categories) {
            const categoriesList = req.query.categories.split(',').map(category => category.trim());

            // Create an array of regex patterns for each category
            const categoryPatterns = categoriesList.map(category =>
            ...
            if (filter.$or) {
                ...
                const categoryOr = categoryPatterns.map(pattern => ({categories: pattern}));
        ...
        if (req.query.fromDate) {
            filter.release_date = {...filter.release_date || {}, $gte: new Date(req.query.fromDate)};
        }

        if (req.query.toDate) {
            filter.release_date = {...filter.release_date || {}, $lte: new Date(req.query.toDate)};
        }
